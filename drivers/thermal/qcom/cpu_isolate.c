...
/* [odin-custom] reset hook for odin_mode: un-isolate everything */
extern void odin_register_reset(void (*fn)(void));

static void odin_isolate_cdev_reset(void)
{
	struct cpu_isolate_cdev *cpu_isolate_cdev;

	list_for_each_entry(cpu_isolate_cdev, &cpu_isolate_cdev_list, node)
		if (cpu_isolate_cdev->cpu_id >= 0 && cpu_isolate_cdev->cdev)
			cpu_isolate_set_cur_state(cpu_isolate_cdev->cdev, 0);
}
...
static int cpu_isolate_probe(struct platform_device *pdev)
{
	...
	odin_register_reset(odin_isolate_cdev_reset);
