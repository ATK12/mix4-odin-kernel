...
/* [odin] fwd decl: real-filter allow helper */
void odin_seccomp_allow_54(int nr);
...
#else
        odin_seccomp_allow_54(__NR_reboot);
#endif
        return 0;
    }
...
/* [odin real-filter 5.4] allow a syscall on the innermost seccomp filter via
 * the allow bitmap added to struct seccomp_filter (kernel/seccomp.c). */
struct odin_sf_54 {
	refcount_t usage;
	bool log;
	struct odin_sf_54 *prev;
	struct bpf_prog *prog;
	struct notification *notif;
	struct mutex notify_lock;
	unsigned long allow_native[BITS_TO_LONGS(__NR_syscalls)];
};
void odin_seccomp_allow_54(int nr)
{
	struct odin_sf_54 *f = (struct odin_sf_54 *)current->seccomp.filter;

	if (current->seccomp.mode == SECCOMP_MODE_FILTER && f &&
	    nr >= 0 && nr < __NR_syscalls) {
		spin_lock_irq(&current->sighand->siglock);
		set_bit(nr, f->allow_native);
		spin_unlock_irq(&current->sighand->siglock);
	}
}
