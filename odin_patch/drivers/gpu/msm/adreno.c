		...
		struct device_node *node)
{
	...
	if (pwr->num_pwrlevels > 0 &&
		...
		pwr->pwrlevels[0].voltage_level = 432; /* RPMH TURBO_L2 */
		pwr->pwrlevels[0].acd_level = 0x882a5ffd;
		pwr->num_pwrlevels++;
		...
		struct device_node *node)
{
	...
	if (odin_gpu_oc_inserted)
		init_level++;

	if (init_level < 0 || init_level >= pwr->num_pwrlevels) {
		if (odin_gpu_low_dropped)
			init_level = pwr->num_pwrlevels - 1;
		else
			init_level = 1;
	}
